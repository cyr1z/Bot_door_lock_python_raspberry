...
from weather_parsers import get_rp5_weather_summary, get_own_measure, \
    YrNow  # , Accuweather
...
def main():
    ...
    def start_message(message):
        ...
        if str(message.chat.id) in data['users']:
            bot.send_sticker(message.chat.id, sticker['start'],
    ...
    def send_text(message):
        if str(message.chat.id) in data['users'] and not old_message(
                message.date):
            if 'открывай' in message.text.lower():
                ...
                yr_now = YrNow(YRCITY)
                # forecast.refresh()
                weather_message: str = get_rp5_weather_summary(RP5CITY,
                                                               LANGRP5)
        ...
        if call.data != '11':
            ...
            try:
                with open("data.json", "w") as write_file:
                    ...
                    bot.send_sticker(call.data, sticker['start'],
                                     reply_markup=keyboard1)
